...
const stuff: number[] | string[] = ["viki"]
...
function greet(whomToGreet: string | string[]): void {
    if (typeof whomToGreet === "string") {
        console.log(`Hello, ${whomToGreet}`)
        return;
    }
    whomToGreet.forEach(name => console.log(`Hello, ${name}`))
}
greet("Viki")
